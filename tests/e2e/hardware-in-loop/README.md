...
- 所有会触发 `dxf.plan.prepare` 的正式 DXF/HIL 脚本统一禁止显式传入已退役的固定参数标识；固定参数 owner 已正式收敛为 runtime 返回的 `production_baseline`
...
- 不得显式传入已退役的固定参数标识，也不得回退到任何全局活动态或最近 artifact 推断；唯一真值为 runtime `production_baseline`
...
- 不得显式传入已退役的固定参数标识；固定参数追溯只能读取 runtime `production_baseline`
...
- 不得显式传入已退役的固定参数标识；validation orchestrator 只汇总 probe 报告返回的 `production_baseline`
