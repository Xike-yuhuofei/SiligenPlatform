#!/usr/bin/env python3
"""dxf-pipeline compatibility service for engineering-data."""
...
from engineering_data.processing.dxf_to_pb import main as canonical_dxf_to_pb_main  # noqa: E402
...
class DXFPreprocessingService:
    ...
    def preprocess_dxf(self,
                       dxf_path: str,
                       config: PreprocessingConfig,
                       output_path: str | None = None) -> PreprocessingResult:
        start_time = time.time()
        pb_path = output_path or f"{dxf_path}.pb"
        exit_code = canonical_dxf_to_pb_main([
            "--input", dxf_path,
            "--output", pb_path,
            "--chordal", str(config.chordal_tolerance),
            "--max-seg", str(config.max_segment_length),
            "--angular", str(config.angular_tolerance),
            "--min-seg", str(config.min_segment_length),
        ])
