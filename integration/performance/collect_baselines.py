...
REPORT_ROOT = ROOT / "integration" / "reports" / "performance"
...
HARDWARE_SMOKE = ROOT / "integration" / "hardware-in-loop" / "run_hardware_smoke.py"
ENGINEERING_REGRESSION = ROOT / "integration" / "scenarios" / "run_engineering_regression.py"
SIMULATED_LINE = ROOT / "integration" / "simulated-line" / "run_simulated_line.py"
DXF_TO_PB = ROOT / "packages" / "engineering-data" / "scripts" / "dxf_to_pb.py"
EXPORT_SIM_INPUT = ROOT / "packages" / "engineering-data" / "scripts" / "export_simulation_input.py"
...
def measure_preprocessing(iterations: int) -> dict[str, Any]:
    ...
    return {
        "fixture": "packages/engineering-contracts/fixtures/cases/rect_diag/rect_diag.dxf",
...
def measure_simulation(iterations: int) -> dict[str, Any]:
    ...
    raw_engine_durations: list[float] = []
    raw_engine_output = ROOT / "packages" / "simulation-engine" / "build" / "Debug" / "simulate_dxf_path.exe"
    ...
    for _ in range(iterations):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            engine_result = run_command(
                [str(raw_engine_output), str(SIM_INPUT), str(output_path)],
                timeout=30,
            )
            if engine_result.return_code != 0:
                raise RuntimeError(f"simulate_dxf_path failed: {engine_result.stderr or engine_result.stdout}")
            raw_engine_durations.append(engine_result.duration_seconds)

    return {
        "fixture": "packages/engineering-contracts/fixtures/cases/rect_diag/simulation-input.json",
        "iterations": iterations,
        "commands": {
            "simulated_line": [sys.executable, str(SIMULATED_LINE)],
            "simulate_dxf_path": [str(raw_engine_output), str(SIM_INPUT), "<temp>/rect_diag.result.json"],
        },
        "metrics": {
            "simulated_line_regression": summarize_samples(simulated_line_durations),
            "simulate_dxf_path_raw": summarize_samples(raw_engine_durations),
        },
        "confidence": {
            "simulated_line_regression": confidence_from_stats(simulated_line_durations),
            "simulate_dxf_path_raw": confidence_from_stats(raw_engine_durations),
        },
    }
...
def measure_reliability(
    ...
    try:
        for _ in range(mock_flow_iterations):
            ...
            ok, payload = protocol.dxf_load(str(DXF_INPUT))
            if not ok:
                cycle_errors.append(str(payload))
            if not protocol.dxf_execute(10.0, dry_run=True, dry_run_speed_mm_s=10.0):
                cycle_errors.append("dxf.execute")
            time.sleep(0.3)
            progress = protocol.dxf_get_progress()
            if not progress.get("running", False) and progress.get("progress", 0.0) <= 0.0:
                cycle_errors.append("dxf.progress")
    ...
    unsupported_methods = ("dxf.resume", "recipe.list")
    try:
    ...
    return {
        ...
        "notes": [
            ...
            "hardware_smoke_hil 直接执行 integration/hardware-in-loop/run_hardware_smoke.py，当前失败被视为已知环境/装配缺口。",
        ],
    }
...
def render_markdown(payload: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        table_line(
            "simulate_dxf_path_raw",
            performance["simulation"]["metrics"]["simulate_dxf_path_raw"],
            performance["simulation"]["confidence"]["simulate_dxf_path_raw"],
        ),
        "",
        "### HMI Startup",
        "",
        ...
        "## Reliability",
        "",
        "| Flow | samples | failures | failure_rate | confidence | error_categories |",
        "|---|---:|---:|---:|---|---|",
    ]
    ...
    for flow in reliability["flows"]:
        ...
        lines.append(
            f"| {flow['flow']} | {flow['samples']} | {flow['failures']} | {flow['failure_rate']:.4f} | "
            f"{flow['confidence']} | {categories} |"
        )
...
def main() -> int:
    ...
    payload = {
        ...
        "monitoring_suggestions": [
            "把 integration/performance/collect_baselines.py 纳入每次重构里程碑或 nightly 执行，并保留 latest.json 与日期版报告。",
