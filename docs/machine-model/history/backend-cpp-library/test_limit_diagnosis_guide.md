...
>
> 当前正式机型事实与排障入口不要以本文为准。需要查看当前 authority 时，优先使用：
>
> - [docs/machine-model/multicard-io-and-wiring-reference.md](../../multicard-io-and-wiring-reference.md)
> - [docs/troubleshooting/canonical-runbook.md](../../../troubleshooting/canonical-runbook.md)
> - [docs/machine-model/backend-cpp-library/06_reference.md](../../backend-cpp-library/06_reference.md)
...
2. **状态位定义**
   - 来自 `src/infrastructure/drivers/multicard/MultiCardCPP.h`
   - `MC_HOME` / `MC_LIMIT_POSITIVE` / `MC_LIMIT_NEGATIVE` / `MC_GPI`

3. **连接配置**
   - 参考 `docs/machine-model/backend-cpp-library/06_reference.md`
   - IP/端口与现场配置保持一致
