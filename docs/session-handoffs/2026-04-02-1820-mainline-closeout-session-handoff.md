...
- 不把 `AGENTS.md.bak` 纳入提交：
  - 该文件是本地备份，不属于可审查交付物。
- 不在本次收尾中修复测试脚本路径问题：
  - `build.ps1` 的 canonical build root 为 `C:\Users\Xike\AppData\Local\SS\cab-528aede677f1`。
  - `apps/runtime-service/run.ps1` 与 `apps/planner-cli/run.ps1` 默认仍查找 `C:\Users\Xike\AppData\Local\SiligenSuite\control-apps-build`。
  - 该问题是验证基建路径不一致，不在本次“整理并发布现有改动”的最小范围内。
...
- `test.ps1 -Suite apps` 当前不能在默认路径假设下全绿，后续若要恢复仓库标准门禁，需要统一 build/test/run 三处对 control-apps build root 的约定。
- 本次 PR 会带上 `AGENTS.md` 的整版重写，评审时需要单独关注仓库级规则替换是否符合团队预期。

## 9. 阻断点

- 无阻断当前提交、推送、创建 draft PR 的权限问题。
- 唯一非绿项为聚合测试入口的既有路径不一致问题，已补充实际 dry-run 证据。
...
- 创建指向 `main` 的 draft PR，并在 PR 描述中说明验证现状与路径门禁问题。
