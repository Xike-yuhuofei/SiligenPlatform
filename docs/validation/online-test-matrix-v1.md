...
- 顶层 canonical 字段固定为 `schema_version`、`started_at`、`finished_at`、`duration_seconds`、`overall_status`、`test_goal`、`scope`、`operator_execution`、`traceability_correspondence`、`observation_alignment`、`observation_integrity`、`control_script_capability`、`steps`、`artifacts`、`snapshot_result`、`log_summary`、`observer_poll_errors`、`observer_poll_failures`、`error`。
- `generated_at`、`input`、`operation_issues` 等别名字段不得出现在正式 report 中。
...
- `steps[]` 只允许 `step`、`status`、`detail`、`timestamp` 四个键，不允许并行 `name` / `details` 风格别名。
