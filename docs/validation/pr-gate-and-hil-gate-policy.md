...
本文档是 `SiligenSuite` PR 合并前自动化测试、native 构建测试、HIL、Nightly 与 Release 门禁的正式策略文档。统一编排入口为 `scripts/validation/invoke-gate.ps1`，具体 gate/step 配置见 `scripts/validation/gates/gates.json`。PR passed 不等于 release ready。
...
| `Nightly Gate` | 夜间 full-offline、performance、长稳/仿真、baseline drift、coverage 和 performance baseline evidence | 定时或受控手动执行 | 阻断 nightly 结论，不直接等同 PR required check | `tests/reports/nightly/` |
| 本地 `pre-push` gate | 开发者本机推送前 | 本地工作站 | 只阻断本机 push | `tests/reports/pre-push-gate/` |
| `Release Gate` | RC / 正式版本发布 | 本地或受控发布环境 | 阻断发布结论 | `tests/reports/releases/<version>/` 或 `tests/reports/releases/orchestrated/` |
...
6. `pyright-static`。
7. `contracts-quick`。
8. HMI/runtime/gateway protocol compatibility quick check。
9. recipe/config schema quick check。
10. state-machine/interlock quick contract。
11. offline simulation smoke。
12. validation/build-system contract quick check。
13. classification evidence generation。

其中 `module-boundary-audit` 是正式 Module Boundary Audit，不再调用旧 `assert-module-boundary-bridges.ps1`。它以 `scripts/validation/boundaries/module-boundaries.json`、`bridge-registry.json`、`boundary-policy.json` 为唯一模型来源，PR 模式按 changed files 审计 CMake/C++/PowerShell 边界依赖。unknown dependency、private surface access、未登记 bridge、未授权 bridge、forbidden dependency、direct workflow target reference、required owner target 缺失、retired surface 重新出现均为阻断项。

该门禁不得承接耗时 native 编译、HIL、performance、release dry-run 或 long-running stability tests，也不得用本地 artifact 替代远程执行结果。native 编译和完整 full-offline 测试由 `Strict Native Gate` 承接。GitHub workflow 只允许 checkout、调用 `invoke-gate.ps1 -Gate pr` 和上传 artifact。
...
- classifier 不可靠时默认 native required。
...
Nightly Gate 用于承接 PR required checks 不适合承担的重验证闭环：

- `full-offline`
- performance suite
- long-running simulation 或当前仓库等价长稳/仿真验证入口
- baseline drift check
- coverage report
- performance baseline evidence

Coverage 当前可以保持 `blocking=false`，但必须在 `gates.json` 中显式声明。Performance、long-running simulation、baseline drift 和 performance baseline evidence 仍按 nightly blocking 语义处理。

## 7. Release Gate

Release Gate 是发布 Go/No-Go 包装，当前可以继续包装 `release-check.ps1`，但发布结论必须额外核对：

- release-check 版本、clean worktree、CHANGELOG。
- native/full-offline 最新证据。
- nightly/performance 最新证据。
- HIL 证据。
- package/dry-run/release manifest 证据。

`Strict PR Gate` 通过只能说明 PR baseline 通过，不说明版本已经可发布。
...
- 测试超时。
- 报告生成失败。
- Module Boundary Audit 报告、coverage、bridge registry 状态或 step logs 缺失。
- Module Boundary Audit 出现 blocking finding。
- `known_failure > 0`。
- 自动化脚本以非零退出码结束。
