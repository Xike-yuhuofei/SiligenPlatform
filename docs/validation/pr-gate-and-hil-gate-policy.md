...
本文档是 `SiligenSuite` PR 合并前自动化测试、native 构建测试与 HIL 门禁的正式策略文档。
...
.\legacy-exit-check.ps1 `
  -Profile CI `
  -ReportDir tests\reports\github-actions\strict-pr-gate\legacy-exit
.\scripts\validation\invoke-semgrep.ps1 `
  -ReportDir tests\reports\github-actions\strict-pr-gate\semgrep
.\scripts\validation\invoke-import-linter.ps1 `
  -ReportDir tests\reports\github-actions\strict-pr-gate\import-linter
```
...
- 改动命中以下路径或语义范围：
  ...
  - `.github/workflows/strict-native-gate.yml`
  - `cmake/`
  - `build.ps1`
  - `ci.ps1`
  - `test.ps1`
  ...
  - `tests/integration/`
  - `tests/e2e/`
  - runtime、gateway、machine、motion、device、adapter、hardware 相关配置。
...
.\ci.ps1 `
  -Suite all `
  -ReportDir tests\reports\github-actions\strict-native-gate `
