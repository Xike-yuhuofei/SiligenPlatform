...
| `hmi-client` | 冻结防回流 | 代码/自动化不得新增 `hmi-client/src`、`hmi-client/scripts` 直连；活动文档出现时必须在白名单内 |
...
| Python 引用 `control-core` 根目录 | `apps/hmi-app/src/hmi_client/ui/main_window.py`、`packages/transport-gateway/tests/test_transport_gateway_compatibility.py`、`tools/migration/validate_device_split.py` | 分别用于 DXF fallback、兼容壳测试、device split 迁移校验 |
| 直接 control-core 兼容壳路径 | `packages/transport-gateway/tests/test_transport_gateway_compatibility.py` | 该测试负责锁定 `control-core/apps/*`、`src/adapters/tcp`、`modules/control-gateway` 仍是 thin shell |
| CMake 硬编码 control-core legacy 依赖 | `packages/device-adapters/CMakeLists.txt`、`packages/shared-kernel/CMakeLists.txt`、`packages/simulation-engine/CMakeLists.txt` | 这些是当前已登记、尚未切完的阻塞项；新增文件一律失败 |
...
4. `hmi-client/src`、`hmi-client/scripts` 是否重新进入代码或自动化链路。
5. `dxf-pipeline/src`、`dxf-pipeline/proto`、`dxf_pipeline` import/CLI 是否重新进入代码或自动化链路。
6. `control-core/build/bin`、`control-core/apps/*`、`control-core/src/adapters/tcp`、`control-core/modules/control-gateway` 是否被未登记文件直接引用。
7. PowerShell 是否在白名单之外重新拼接 `control-core` 根目录。
8. Python 是否在白名单之外重新依赖 `control-core` 根目录。
9. CMake 是否在白名单之外新增 `control-core` include/link/source 依赖。
10. 活动文档是否在白名单之外重新把 legacy 路径写回默认入口说明。

## 6. 还无法自动化的门禁

1. 仓库外 sibling repo 或现场脚本是否仍直接依赖 `hmi-client`、`dxf-pipeline`、`control-core`。
2. `apps/control-runtime`、`apps/control-cli`、`apps/control-tcp-server` 是否已经完成“真实产物切换”，而不仅是 wrapper 存在。
3. 外部用户是否仍通过旧 Python 包名、旧 CLI 名称或手工脚本依赖兼容壳。
4. 文档语义是否真的把 canonical 说清楚。当前自动化只能限制 legacy 路径出现位置，不能理解整段语义。
5. HIL、现场部署、回滚链路是否已经不再需要 `control-core` / `dxf-pipeline` 兼容产物。
...
4. 若准备删除 `hmi-client` 或 `dxf-pipeline`，需额外确认外部消费者已经迁移，且至少经历一个兼容观察周期。
