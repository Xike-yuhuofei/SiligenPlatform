...
4. Worker 再拉取 snapshot
   ...
   runtime: trajectory_points
         ...
         -> 返回 preview_kind=glue_points / glue_points / motion_preview
   runtime -> gateway -> TcpClient -> Worker
...
5. HMI 落地快照并渲染
   ...
   MainWindow -> _render_runtime_preview_html(preview_source, glue_points, motion_preview)
   MainWindow: 以 `planned_glue_snapshot + glue_points` 作为成功主语义，`motion_preview` 仅用于点胶头运动轨迹观察，不重算轨迹
...
2. 联调定位“预览显示异常”时，先区分是 HMI 渲染问题，还是 `motion_preview` 上游返回问题。
