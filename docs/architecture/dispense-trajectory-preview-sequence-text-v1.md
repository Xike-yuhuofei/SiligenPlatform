...
4. Worker 再拉取 snapshot
   ...
   runtime: trajectory_points
         ...
         -> 返回 snapshot_id / snapshot_hash / preview_source / trajectory_polyline
   runtime -> gateway -> TcpClient -> Worker
...
5. HMI 落地快照并渲染
   ...
   MainWindow: 校验 snapshot_id / snapshot_hash / trajectory_polyline
   ...
   MainWindow -> _render_runtime_preview_html(preview_source, trajectory_polyline)
   MainWindow: 仅做坐标映射 + SVG 点绘制，不重算轨迹
   MainWindow: 若 preview_source == mock_synthetic，则显式提示“模拟轨迹，非真实几何”
...
- `preview_source == runtime_snapshot`
  - HMI 将其标识为“运行时快照”，可进入真实轨迹验收口径。
- `preview_source == mock_synthetic`
  - HMI 必须给出“模拟轨迹，非真实几何”强提示。
  - 该结果可用于联调和 smoke，但不得作为真实轨迹外观签收依据。
...
2. 联调定位“预览显示异常”时，先区分是 HMI 渲染问题，还是 `trajectory_polyline` 上游返回问题。
3. 联调定位“启动被阻断”时，优先检查 gate 状态、`snapshot_hash`、`plan_fingerprint` 和互锁状态。
