...
4. Worker 再拉取 snapshot
   ...
   runtime: trajectory_points
         ...
         -> 返回 preview_kind=glue_points / glue_points / execution_polyline
   runtime -> gateway -> TcpClient -> Worker
...
5. HMI 落地快照并渲染
   ...
   MainWindow -> _render_runtime_preview_html(preview_source, glue_points, execution_polyline)
   MainWindow: 以 `planned_glue_snapshot + glue_points` 作为成功主语义，`execution_polyline` 仅用于辅助叠加，不重算轨迹
