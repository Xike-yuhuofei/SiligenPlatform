...
> 本文档已转为历史进展记录，保留 control apps 从 wrapper 过渡到根级真实入口时的阶段性说明与实现细节。
>
> 当前正式状态请以 [docs/architecture/legacy-cutover-status.md](../../legacy-cutover-status.md) 为准；验收类历史证据请查看 [docs/architecture/history/closeouts/control-apps-binary-cutover.md](../closeouts/control-apps-binary-cutover.md)。
>
> 下文中的“当前状态”“已完成”等表述均指 `2026-03-19` 当时的推进阶段，不再直接代表当前正式真值。

## 1. 目标与边界

本文记录 `apps/control-runtime`、`apps/control-tcp-server`、`apps/control-cli` 从 wrapper 过渡到真实入口的当前状态。

当前 control apps 的正式状态以 `docs/architecture/legacy-cutover-status.md` 为准；历史 cutover 记录已转移到 `docs/architecture/history/closeouts/control-apps-binary-cutover.md`。本文保留为补充说明，并与正式状态页保持一致。
...
- 历史 cutover 证据见 `docs/architecture/history/closeouts/control-cli-cutover.md`
