...
| A07 | 轨迹生成链 | `ProcessPath`、动态参数、插补策略 -> `MotionPlan` / `MotionTrajectory` | 把“要走哪条工艺路径”转成“如何按速度、加速度、jerk 约束去走”的时间化运动描述。`Ruckig`、`InterpolationAlgorithm` 都属于这一层。 | 板卡 API 调用链、HMI 预览渲染链 |
| A08 | 插补程序合成链 | `ProcessPath` + `MotionTrajectory` -> `InterpolationData` / `interpolation_segments` | 把上位机的几何语义与时间律语义合成为板卡可执行的段级插补程序。 | 板卡执行链、状态回流链 |
...
## 5. A07-A08-A12 的运动控制细分

“运动控制链路”在项目内不是单点概念，而是以下三条相邻子链的组合：

| 子链 | 权威输入 -> 输出 | 精准定义 | 主要依据 |
|---|---|---|---|
| A07-1 轨迹生成子链 | `ProcessPath` / 几何路径 -> `MotionTrajectory` / `TrajectoryPoint` | 几何与工艺路径的时间参数化；`Ruckig` 在此层承担时间律生成器，不是板卡插补器。 | `modules/workflow/application/usecases/motion/interpolation/README.md`、`docs/architecture/dxf-motion-execution-contract-v1.md` |
| A08-1 程序合成子链 | `ProcessPath` + `MotionTrajectory` -> `InterpolationData` | 上位机把几何语义和速度语义合成为板卡段程序；这是板卡执行前的最后一层上位机真源。 | `docs/architecture/dxf-motion-execution-contract-v1.md` |
...
1. `Ruckig` 属于 **轨迹生成子链**。
2. `InterpolationAlgorithm` 属于 **轨迹生成子链**。
3. `InterpolationProgramPlanner` 属于 **程序合成子链**。
4. `InterpolationAdapter -> MultiCard SDK` 才属于 **板卡执行子链**。
...
例如 S9“运动轨迹生成”是正式流程阶段，但在工程实现上至少包含：

1. A07 轨迹生成链
2. A08 插补程序合成链
3. 与 B03 预览链共享的快照生成分支
...
3. `Ruckig`、插补算法、`MotionTrajectory` 属于 **轨迹生成层**，不等于板卡执行层。
