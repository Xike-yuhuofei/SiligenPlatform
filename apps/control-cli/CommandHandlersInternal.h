...
std::string ResolveCliConfigPath(const std::string& preferred_path = "") {
    ...
    const std::string canonical = "config/machine/machine_config.ini";
    if (std::filesystem::exists(canonical)) {
        return canonical;
    }

    return "config/machine_config.ini";
}
...
constexpr const char* kFallbackDxfFilePath = "examples/dxf/rect_diag.dxf";
