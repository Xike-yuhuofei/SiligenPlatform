...
void SignalHandler(int signal) {
    std::cout << "\n[control-runtime] 收到信号 " << signal << "，准备退出..." << std::endl;
...
void PrintUsage() {
    std::cout
        << "用法: siligen_control_runtime [选项]\n"
        << "选项:\n"
        << "  -c, --config <路径>      机器配置文件路径 (默认: config/machine/machine_config.ini)\n"
...
int main(int argc, char* argv[]) {
    try {
        ...
        if (options.version) {
            std::cout << "siligen_control_runtime git=" << SILIGEN_GIT_HASH << std::endl;
            return 0;
        }
        ...
        std::cout << "[control-runtime] 已完成宿主装配" << std::endl;
        std::cout << "[control-runtime] config=" << options.config_path << std::endl;
        std::cout << "[control-runtime] log=" << options.log_path << std::endl;
        std::cout << "[control-runtime] task_threads=" << options.task_threads << std::endl;
        std::cout << "[control-runtime] git=" << SILIGEN_GIT_HASH << std::endl;

        if (options.run_seconds > 0) {
            std::this_thread::sleep_for(std::chrono::seconds(options.run_seconds));
            std::cout << "[control-runtime] 已按 --run-seconds 退出" << std::endl;
            return 0;
        }

        std::cout << "[control-runtime] 运行中，按 Ctrl+C 停止" << std::endl;
        std::unique_lock<std::mutex> lock(g_shutdown_mutex);
        g_shutdown_cv.wait(lock, [] { return !g_keep_running.load(); });
        container.reset();
        std::cout << "[control-runtime] 已停止" << std::endl;
        return 0;
    } catch (const std::exception& ex) {
        std::cerr << "[control-runtime] 错误: " << ex.what() << std::endl;
        return 1;
    } catch (...) {
        std::cerr << "[control-runtime] 未知错误" << std::endl;
        return 2;
    }
}
