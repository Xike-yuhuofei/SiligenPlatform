...
$legacyCandidates = @(
    (Join-Path $controlCoreRoot "build\bin\siligen_tcp_server.exe"),
    (Join-Path $controlCoreRoot "build\bin\Debug\siligen_tcp_server.exe"),
    (Join-Path $controlCoreRoot "build\bin\Release\siligen_tcp_server.exe"),
    (Join-Path $controlCoreRoot "build\bin\RelWithDebInfo\siligen_tcp_server.exe")
)
...
if ($DryRun) {
    if ($UseLegacyFallback -and $legacyResolved) {
        Write-Output "control-tcp-server target: $legacyResolved"
...
if ($legacyResolved) {
    Write-Error "找到 legacy siligen_tcp_server.exe ('$legacyResolved')，但默认已禁用回退。请先执行根级 build 生成 canonical 产物，或在确需审计式回退时显式传入 -UseLegacyFallback。"
}
