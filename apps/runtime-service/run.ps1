...
if (-not $exePath) {
    if ($controlAppsBuildRoots.Count -eq 0) {
        ...
        throw "current-workspace control-apps build root not ready for runtime-service; searched: '$searchRootsText'"
    }
    $rootsText = ($controlAppsBuildRoots -join "', '")
    throw "runtime-service executable not found under current-workspace control-apps roots: '$rootsText'"
}
