...
constexpr const char* kErrorSource = "WorkflowMotionControlOperationsBridge";
...
WorkflowMotionOperationsBundle CreateWorkflowMotionOperations(
    ...
    WorkflowMotionOperationsBundle bundle;
    bundle.homing_operations =
        std::make_shared<WorkflowMotionHomingOperations>(std::move(home_use_case), std::move(ensure_ready_zero_use_case));
    bundle.manual_operations = std::make_shared<WorkflowMotionManualOperations>(std::move(manual_use_case));
    bundle.monitoring_operations =
        std::make_shared<WorkflowMotionMonitoringOperations>(std::move(monitoring_use_case));
    return bundle;
}
