...
WorkflowRuntimeSupervisionBackend::WorkflowRuntimeSupervisionBackend(
    ...
    std::shared_ptr<Siligen::Application::UseCases::Dispensing::DispensingWorkflowUseCase> dispensing_workflow_use_case,
...
Result<RuntimeSupervisionInputs> WorkflowRuntimeSupervisionBackend::ReadInputs() const {
    ...
    if (dispensing_workflow_use_case_) {
        ...
        inputs.interlock_latched = dispensing_workflow_use_case_->IsInterlockLatched();
    }

    if (dispensing_execution_use_case_) {
        inputs.active_job_id = dispensing_execution_use_case_->GetActiveJobId();
        if (!inputs.active_job_id.empty() && dispensing_workflow_use_case_) {
            auto job_status_result = dispensing_workflow_use_case_->GetJobStatus(inputs.active_job_id);
