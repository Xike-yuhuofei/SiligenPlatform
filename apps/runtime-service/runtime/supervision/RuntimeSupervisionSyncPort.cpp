...
constexpr const char* kErrorSource = "RuntimeSupervisionSyncPort";
...
RuntimeSupervisionSyncPort::RuntimeSupervisionSyncPort(
...
void RuntimeSupervisionSyncPort::RememberObservedJob(const std::string& job_id) const {
    std::lock_guard<std::mutex> lock(observed_job_mutex_);
    last_observed_job_id_ = job_id;
}

void RuntimeSupervisionSyncPort::ClearObservedJob() const {
    std::lock_guard<std::mutex> lock(observed_job_mutex_);
    last_observed_job_id_.clear();
}

void RuntimeSupervisionSyncPort::SyncTerminalJobIfNeeded() const {
...
Result<RuntimeSupervisionSnapshot> RuntimeSupervisionSyncPort::ReadSnapshot() const {
