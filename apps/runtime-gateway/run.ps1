...
if (-not $exePath) {
    if ($controlAppsBuildRoots.Count -eq 0) {
        ...
        throw "current-workspace control-apps build root not ready for runtime-gateway; searched: '$searchRootsText'"
    }
    $rootsText = ($controlAppsBuildRoots -join "', '")
    throw "runtime-gateway executable not found under current-workspace control-apps roots: '$rootsText'"
}
...
if ($DryRun) {
    Write-Output "DRYRUN app=runtime-gateway exe=$exePath config=$resolvedConfigPath vendor_dir=$resolvedVendorDir preflight_skipped=$SkipPreflight log=logs/tcp_server.log"
    exit 0
}
