...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if ((arg == "--port" || arg == "-p") && i + 1 < argc) {
            ...
            std::cout << "用法: siligen_runtime_gateway [选项]\n"
                      << "选项:\n"
                      << "  -p, --port <端口>     TCP监听端口 (默认: 9527)\n"
                      << "  -c, --config <路径>   配置文件路径 (默认: "
    ...
    try {
        ...
        auto runtimeSupervisionPort =
            container->ResolvePort<Siligen::RuntimeExecution::Contracts::System::IRuntimeSupervisionPort>();
        auto hardwareMode = Siligen::Shared::Types::HardwareMode::Real;
        if (configPort) {
            auto hardwareModeResult = configPort->GetHardwareMode();
            if (hardwareModeResult.IsSuccess()) {
                hardwareMode = hardwareModeResult.Value();
            }
        }
        Siligen::Infrastructure::Hardware::MockMultiCardWrapper* mockMultiCard = nullptr;
        if (hardwareMode == Siligen::Shared::Types::HardwareMode::Mock) {
            mockMultiCard = static_cast<Siligen::Infrastructure::Hardware::MockMultiCardWrapper*>(
                container->GetMultiCardInstance());
        }
        auto mockIoControl = std::make_shared<Siligen::Adapters::Tcp::MockIoControlService>(
            resolved_config_path,
            mockMultiCard,
            hardwareMode == Siligen::Shared::Types::HardwareMode::Mock);
