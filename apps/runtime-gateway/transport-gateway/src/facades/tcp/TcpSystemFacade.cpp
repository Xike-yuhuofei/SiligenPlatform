...
Shared::Types::Result<bool> TcpSystemFacade::IsInEmergencyStop() const {
    if (!emergency_stop_use_case_) {
        return Shared::Types::Result<bool>::Failure(
            Shared::Types::Error(Shared::Types::ErrorCode::PORT_NOT_INITIALIZED, "EmergencyStopUseCase not available"));
    }
    return emergency_stop_use_case_->IsInEmergencyStop();
}

Shared::Types::Result<void> TcpSystemFacade::RecoverFromEmergencyStop() {
    if (!emergency_stop_use_case_) {
        return Shared::Types::Result<void>::Failure(
            Shared::Types::Error(Shared::Types::ErrorCode::PORT_NOT_INITIALIZED, "EmergencyStopUseCase not available"));
    }
    return emergency_stop_use_case_->RecoverFromEmergencyStop();
}
