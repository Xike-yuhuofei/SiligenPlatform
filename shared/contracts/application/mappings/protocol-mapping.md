...
| `dxf.preview.snapshot` | 命令 | `CommandProtocol.dxf_preview_snapshot()` | `HandleDxfPreviewSnapshot` | 无独立 CLI facade | `snapshot_hash` / `plan_id` / `preview_kind` / `glue_points` / `execution_polyline` / `motion_preview` / `preview_validation_classification` / `preview_exception_reason` / `preview_failure_reason` / `preview_diagnostic_code` | 唯一 shared authority success shape 是 `preview_source=planned_glue_snapshot` + `preview_kind=glue_points` + 非空 `glue_points`；`motion_preview` 用于头部运动轨迹预览，`trajectory_polyline` 与 `polyline_*` 仅保留兼容别名 |
...
- `dxf.preview.snapshot` 的唯一成功主预览语义是 shared authority 下的 `planned_glue_snapshot + glue_points`；`motion_preview` 仅用于头部运动轨迹预览，`execution_polyline` 只用于 compat 辅助叠加，不参与 authority 判定。
