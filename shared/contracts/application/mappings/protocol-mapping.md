...
| 方法 | 类型 | HMI 调用点 | TCP 处理器 | CLI 对应语义 | 结果字段 | 兼容说明 |
|---|---|---|---|---|---|---|
| `dxf.artifact.create` | 命令 | `CommandProtocol.dxf_create_artifact()` | `HandleDxfArtifactCreate` | 无独立 CLI facade | `artifact_id` / `filepath` / `prepared_filepath` / `size` / `import_result_classification` / `import_production_ready` | 生成 canonical artifact，作为后续 prepare 的唯一输入；旧 `dxf.load` 已退役，不再保留兼容入口 |
| `dxf.plan.prepare` | 命令 | `CommandProtocol.dxf_prepare_plan()` | `HandleDxfPlanPrepare` | `DXF_PLAN` | `plan_id` / `plan_fingerprint` / `prepared_filepath` / `estimated_time_s` / `import_result_classification` / `import_production_ready` / `preview_validation_classification` / `preview_exception_reason` / `preview_failure_reason` / `preview_diagnostic_code` | 必须显式传入 `artifact_id` / `recipe_id` / `version_id`；`recipe_id` / `version_id` 必须指向 `published` recipe version。可选 `requested_execution_strategy` 未传时默认按 `flying_shot` 解析。`POINT` 若无 formal carrier trajectory，仅允许基于 recipe-owned `point_flying_carrier_policy` 并按 `approach_direction = normalize(point - planning_start_position)` 合成 formal carrier trajectory；禁止 `artifact_id`、active recipe、activeVersionId 或全局活动态回退 |
| `dxf.preview.snapshot` | 命令 | `CommandProtocol.dxf_preview_snapshot()` | `HandleDxfPreviewSnapshot` | 无独立 CLI facade | `snapshot_hash` / `plan_id` / `preview_kind` / `glue_points` / `glue_reveal_lengths_mm` / `execution_polyline` / `motion_preview` / `preview_validation_classification` / `preview_exception_reason` / `preview_failure_reason` / `preview_diagnostic_code` | 唯一 shared authority success shape 是 `preview_source=planned_glue_snapshot` + `preview_kind=glue_points` + 非空 `glue_points`；`glue_reveal_lengths_mm` 与 `glue_points` 等长同序，作为胶点 reveal 真值；`motion_preview` 用于头部运动轨迹预览，`trajectory_polyline` 与 `polyline_*` 仅保留兼容别名 |
...
| `dxf.job.start` | 命令 | `CommandProtocol.dxf_start_job()` | `HandleDxfJobStart` | `DXF_DISPENSE` | `started` / `job_id` / `plan_id` / `plan_fingerprint` / `import_result_classification` / `import_production_ready` | canonical 启动入口；旧 `dxf.execute` 已退役；仅在 preview confirmed、source valid、authority shared 且导入结果 production-ready 时允许启动 |
...
- `dxf.preview.snapshot` 的唯一成功主预览语义是 shared authority 下的 `planned_glue_snapshot + glue_points + glue_reveal_lengths_mm`；`motion_preview` 仅用于头部运动轨迹预览，`execution_polyline` 只用于 compat 辅助叠加，不参与 authority 判定。
