...
- `status` 的 owner 数据面当前来自 `IRuntimeStatusPort` snapshot；其中 `supervision` / `effective_interlocks` / `io` 继续由内嵌 `IRuntimeSupervisionPort` snapshot 提供，`runtime-gateway` 只承担协议序列化和 `machine_state` compat 投影。
