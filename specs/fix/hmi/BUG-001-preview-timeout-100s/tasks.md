...
- [X] T001 在 `D:\Projects\SiligenSuite\modules\hmi-application\application\hmi_application\preview_session.py` 定义 DXF 打开自动预览专用的 `100.0s` timeout policy 常量与 worker 作用域注释
...
**Goal**: 让 DXF 打开自动预览链路在 prepare/snapshot 处理时间大于 `15s` 但小于 `100s` 时继续等待并成功展示预览

**Independent Test**: 打开一个预览生成时间大于 `15s` 且小于 `100s` 的有效 DXF，验证自动预览链路不会在 `15.0s` 时提前失败，并能继续展示 authority preview payload

### Tests for User Story 1

> 先写这些回归，并确认它们在实现前失败

- [X] T006 [P] [US1] 在 `D:\Projects\SiligenSuite\modules\hmi-application\tests\unit\test_preview_session.py` 添加 `PreviewSnapshotWorker.run()` 对 `dxf.plan.prepare` 和 `dxf.preview.snapshot` 都使用 `100.0s` 的回归
...
- [X] T008 [US1] 在 `D:\Projects\SiligenSuite\modules\hmi-application\application\hmi_application\preview_session.py` 将 `PreviewSnapshotWorker.run()` 改为调用 `dxf_prepare_plan(..., timeout=100.0)` 和 `dxf_preview_snapshot(..., timeout=100.0)`
...
**Goal**: 让自动预览链路只在真实超过 `100s` 时才返回 timeout，并确保用户可见错误明确反映 `100.0s`，同时保留非 timeout 失败原语义

**Independent Test**: 打开一个超过 `100s` 的有效 DXF，验证 timeout 只在完整等待窗口后出现且提示包含 `100.0s`；再验证非 timeout 错误仍按原文透传

### Tests for User Story 2

> 先写这些回归，并确认它们在实现前失败

- [X] T010 [P] [US2] 在 `D:\Projects\SiligenSuite\apps\hmi-app\tests\unit\test_main_window.py` 添加 timeout 失败显示 `Request timed out (100.0s)` 且非 timeout 失败保留原始 detail 的 UI 回归
...
- `D:\Projects\SiligenSuite\modules\hmi-application\application\hmi_application\preview_session.py` 是 worker owner 文件，`US1` 与 `US2` 都依赖它来限定 100 秒作用范围
...
2. 交付 `US1`，解决“有效 DXF 在 15-100 秒之间仍能成功预览”
3. 交付 `US2`，解决“只有真正超过 100 秒才 timeout 且文案正确”
...
- `100.0s` 只属于“打开 DXF 自动预览”链路，不应被默认扩散到 runtime resync、`dxf.preview.confirm`、`dxf.job.start`、`connect` 或 `dxf.load`
