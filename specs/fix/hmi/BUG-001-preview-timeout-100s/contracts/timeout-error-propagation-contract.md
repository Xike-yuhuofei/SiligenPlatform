...
定义 timeout 错误从 transport 到用户可见 UI 的传播约束，确保当自动预览真正超过 `300s` 时，用户看到的是反映新阈值的超时信息，而不是过期的 `15.0s` 文案。
...
3. worker、UI 和 preview session 不得再单独维护一个“15 秒”或“300 秒”的平行文案源。
...
- `Request timed out (300.0s)`
- 语义等价、明确体现 300 秒上限的用户可见超时信息
...
1. 本地请求已经等待到 `300s`，但 UI 仍显示 `15.0s`。
