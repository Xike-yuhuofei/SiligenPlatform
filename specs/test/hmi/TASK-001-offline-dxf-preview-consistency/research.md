...
## Decision 2: 权威在线预览直接复用现有 `dxf.preview.snapshot -> planned_glue_snapshot + glue_points` 链路

- Decision: 不再设计额外的本地权威提供者，也不引入新的本地 fallback。本特性的权威在线预览直接复用现有 runtime/gateway 主链：真实 DXF 进入 `dxf.artifact.create` / `dxf.plan.prepare` 后，由 `dxf.preview.snapshot` 返回 `planned_glue_snapshot + glue_points`。
- Rationale: 仓内正式协议、gateway 守门和 HMI 门禁已经围绕 `planned_glue_snapshot + glue_points` 收口。继续把旧版 `runtime_snapshot` 视为成功来源只会保留语义分叉和验收歧义。
- Alternatives considered:
...
## Decision 4: 来源分类保持简单，真实验收只接受 `planned_glue_snapshot`

- Decision: 在线真实预览验收只接受 `preview_source == planned_glue_snapshot` 且 `preview_kind == glue_points`。旧版 `runtime_snapshot`、`mock_synthetic`、来源缺失、来源不明、历史缓存或与当前 DXF / 当前 plan 无法回链的结果，一律视为非通过。
- Rationale: 当前 live contract 已把 `planned_glue_snapshot + glue_points` 冻结为唯一成功形态，`runtime_snapshot` 只保留给旧链诊断。验收边界必须与 live chain 保持一致。
- Alternatives considered:
...
- Rationale: 这组资产已经对权威 preview 的点数、范围、关键采样点和几何摘要形成闭环，是当前仓内最稳定、最可追溯的真实 DXF 预览基线。
- Alternatives considered:
...
- Rationale: 在线 smoke 证明 HMI 到达 `online_ready`；协议测试保证 `preview_source/preview_kind/glue_points` 契约不漂移；first-layer 几何回归保证 canonical DXF 的形状稳定；HIL 快照脚本负责生成真实运行证据并校验 `planned_glue_snapshot + glue_points`。
- Alternatives considered:
