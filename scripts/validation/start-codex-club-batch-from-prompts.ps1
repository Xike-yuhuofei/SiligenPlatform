...
param(
    ...
    [ValidateSet("NewWindow", "NewTab")]
    [string]$LaunchMode = "NewWindow",
...
if (-not (Test-Path $promptDirectoryFull)) {
    throw "Prompt 目录不存在：$promptDirectoryFull"
}
...
for ($i = $StartId; $i -le $EndId; $i++) {
    $promptFiles += Join-Path $promptDirectoryFull ("T{0:D3}.md" -f $i)
}

$missing = $promptFiles | Where-Object { -not (Test-Path $_) }
if ($missing.Count -gt 0) {
    throw ("以下 Prompt 文件缺失：`n" + ($missing -join "`n"))
}
...
foreach ($file in $promptFiles) {
    ...
    $promptBash = Convert-ToBashPath -Path $file
    $logFile = Join-Path $LogDir ("codex-club-{0}-{1}.log" -f $task, $batchTimestamp)
    $logBash = Convert-ToBashPath -Path $logFile
    $tempScriptWin = Join-Path ([System.IO.Path]::GetTempPath()) ("codex-club-{0}-{1}.sh" -f $task, $batchTimestamp)
...
foreach ($item in $results) {
    Write-Output ("LAUNCHED {0} | LOG={1}" -f $item.Task, $item.LogFile)
}
