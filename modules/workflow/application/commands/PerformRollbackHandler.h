#pragma once

#include "../../services/rollback/PerformRollbackService.h"

#include <utility>

namespace Siligen::Workflow::Application::Commands {

class PerformRollbackHandler {
   public:
    [[nodiscard]] Domain::WorkflowRun Handle(
        Domain::WorkflowRun workflow_run,
        const Domain::RollbackDecision& decision,
        const Contracts::PerformRollback& command,
        std::int64_t occurred_at) const {
        return service_.Apply(std::move(workflow_run), decision, command, occurred_at);
    }

   private:
    Services::Rollback::PerformRollbackService service_{};
};

}  // namespace Siligen::Workflow::Application::Commands
