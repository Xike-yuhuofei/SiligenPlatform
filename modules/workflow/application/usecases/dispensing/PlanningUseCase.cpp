...
WorkflowAuthorityPreviewRequest BuildAuthorityPreviewInput(
    const PlanningRequest& request,
    ...
    const std::shared_ptr<Siligen::Domain::Configuration::Ports::IConfigurationPort>& config_port) {
    WorkflowAuthorityPreviewRequest input;
    ...
    input.runtime_options.dispensing_velocity = runtime_params.dispensing_velocity;
    input.runtime_options.acceleration = runtime_params.acceleration;
    input.runtime_options.dispenser_interval_ms = runtime_params.dispenser_interval_ms;
    input.runtime_options.dispenser_duration_ms = runtime_params.dispenser_duration_ms;
    input.runtime_options.trigger_spatial_interval_mm = runtime_params.trigger_spatial_interval_mm;
    input.runtime_options.valve_response_ms = runtime_params.valve_response_ms;
    input.runtime_options.safety_margin_ms = runtime_params.safety_margin_ms;
    input.runtime_options.min_interval_ms = runtime_params.min_interval_ms;
    input.runtime_options.sample_dt = runtime_params.sample_dt;
    input.runtime_options.sample_ds = runtime_params.sample_ds;
    input.runtime_options.spline_max_step_mm = request.spline_max_step_mm;
    input.runtime_options.spline_max_error_mm = request.spline_max_error_mm;
    input.runtime_options.compensation_profile = runtime_params.compensation_profile;
...
WorkflowExecutionAssemblyRequest BuildExecutionAssemblyInput(
    const PlanningRequest& request,
    const PreparedAuthorityPreview& authority_preview,
    MotionPlan motion_plan,
    const PreviewRuntimeParams& runtime_params) {
    WorkflowExecutionAssemblyRequest input;
...
PlanningUseCase::PlanningUseCase(
    ...
    : path_source_(std::move(path_source)),
      ...
      planning_assembly_operations_(
          planning_assembly_operations
              ? std::move(planning_assembly_operations)
...
Result<PreparedAuthorityPreview> PlanningUseCase::PrepareAuthorityPreview(const PlanningRequest& request) {
    ...
    auto authority_result = planning_assembly_operations_->BuildAuthorityPreviewArtifacts(
...
Result<ExecutionAssemblyResponse> PlanningUseCase::AssembleExecutionFromAuthority(
    ...
    auto assembly_result = planning_assembly_operations_->BuildExecutionArtifactsFromAuthority(assembly_input);
    ...
    response.export_request = assembly_result.Value().export_request;
