...
Result<void> ValidateRuntimeOverridesExplicit(const DispensingRuntimeOverrides& overrides) {
    ...
    if (overrides.dry_run) {
        if (overrides.machine_mode.has_value() &&
            overrides.machine_mode.value() != Siligen::Domain::Machine::ValueObjects::MachineMode::Test) {
        ...
        if (overrides.execution_mode.has_value() &&
            overrides.execution_mode.value() != ValueObjects::JobExecutionMode::ValidationDryCycle) {
        ...
        if (overrides.output_policy.has_value() &&
            overrides.output_policy.value() != ValueObjects::ProcessOutputPolicy::Inhibited) {
...
Result<DispensingRuntimeParams> DispensingProcessService::BuildRuntimeParams(
    ...
    if (config_port_) {
        ...
        if (config_result.IsSuccess()) {
            ...
            SILIGEN_LOG_INFO_FMT_HELPER(
                "BuildRuntimeParams config snapshot: max_speed=%.3f, max_acceleration=%.3f, pulse_per_mm=%.3f",
                config.machine.max_speed,
                config.machine.max_acceleration,
                config.machine.pulse_per_mm);
        } else {
    ...
    const auto execution_mode =
        overrides.execution_mode.value_or(overrides.dry_run
                                              ? ValueObjects::JobExecutionMode::ValidationDryCycle
                                              : ValueObjects::JobExecutionMode::Production);
    SILIGEN_LOG_INFO(
        "BuildRuntimeParams overrides: dry_run=" + std::string(overrides.dry_run ? "true" : "false") +
        ", execution_mode=" + std::string(ValueObjects::ToString(execution_mode)) +
        ", dispensing_speed_mm_s=" +
        (overrides.dispensing_speed_mm_s.has_value() ? std::to_string(*overrides.dispensing_speed_mm_s) : std::string("<none>")) +
        ", dry_run_speed_mm_s=" +
        (overrides.dry_run_speed_mm_s.has_value() ? std::to_string(*overrides.dry_run_speed_mm_s) : std::string("<none>")) +
        ", rapid_speed_mm_s=" +
        (overrides.rapid_speed_mm_s.has_value() ? std::to_string(*overrides.rapid_speed_mm_s) : std::string("<none>")) +
        ", acceleration_mm_s2=" +
        (overrides.acceleration_mm_s2.has_value() ? std::to_string(*overrides.acceleration_mm_s2) : std::string("<none>")));
    float32 velocity = default_velocity;
    if (execution_mode == ValueObjects::JobExecutionMode::ValidationDryCycle) {
    ...
    SILIGEN_LOG_INFO_FMT_HELPER("BuildRuntimeParams selected_velocity=%.3f", velocity);
...
Result<DispensingExecutionReport> DispensingProcessService::ExecutePlanInternal(
    ...
    const auto dispensing_outputs_enabled =
        options.dispense_enabled && options.guard_decision.AllowsDispensingOutputs();
    if (!options.guard_decision.allow_motion) {
        return Result<DispensingExecutionReport>::Failure(
            Error(ErrorCode::INVALID_STATE, "安全护栏禁止当前运动执行", "DispensingProcessService"));
    }

    if (dispensing_outputs_enabled && !valve_port_) {
        return Result<DispensingExecutionReport>::Failure(
    ...
    controller_config.use_hardware_trigger = options.use_hardware_trigger && dispensing_outputs_enabled;
    ...
    if (dispensing_outputs_enabled && trigger_output.trigger_positions.empty()) {
        return Result<DispensingExecutionReport>::Failure(
    ...
    while (cursor < total_segments) {
        auto pause_result =
            WaitWhilePaused(stop_flag, pause_flag, pause_applied_flag, dispensing_outputs_enabled, observer);
