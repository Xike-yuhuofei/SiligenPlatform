    ...
    Result<void> Validate() const noexcept {
        ...
        if (dry_run) {
            if (machine_mode.has_value() &&
                ...
                return Result<void>::Failure(
                    Error(ErrorCode::INVALID_PARAMETER, "dry_run与machine_mode语义冲突"));
            }
            if (execution_mode.has_value() &&
                execution_mode.value() != JobExecutionMode::ValidationDryCycle) {
                return Result<void>::Failure(
                    Error(ErrorCode::INVALID_PARAMETER, "dry_run与execution_mode语义冲突"));
            }
            if (output_policy.has_value() &&
                output_policy.value() != ProcessOutputPolicy::Inhibited) {
                return Result<void>::Failure(
                    Error(ErrorCode::INVALID_PARAMETER, "dry_run与output_policy语义冲突"));
            }
        }
