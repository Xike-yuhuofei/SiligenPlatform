...
- machine/calibration 当前的 live workflow residue 已从旧 bridge-domain concrete 迁移为 public-surface split 问题：`domain/include/domain/machine/aggregates/DispenserModel.h` 与 `domain/include/domain/machine/domain-services/CalibrationProcess.h` 已 forward 到 `modules/coordinate-alignment/domain/machine/**`，但 `IHardwareConnectionPort`、`IHardwareTestPort`、`MachineMode`、`MachineState` 仍由 workflow public include root 暴露并存在 live consumer。
...
- round `25` 已完成 `WF-B03` 当前最小 producer/export slice：`siligen_workflow_application_headers` 已删除 `${SILIGEN_SHARED_COMPAT_INCLUDE_ROOT}` 与全部 foreign `INTERFACE` links，`siligen_application_dispensing` 已显式 `PUBLIC` 链接 `siligen_process_path_contracts_public`、`siligen_motion_planning_contracts_public`、`siligen_shared_runtime_contracts` 与 `siligen_runtime_execution_runtime_contracts`。
...
- **`observed_fact`**：`siligen_workflow_domain_headers` 当前已只暴露 workflow 自身 include root；round `25` 又确认 `siligen_workflow_application_headers` 已删除 `${SILIGEN_SHARED_COMPAT_INCLUDE_ROOT}` 与全部 foreign `INTERFACE` links；round `26/27` 又确认 live app consumer 与 workflow canonical/integration tests 已不再依赖 `siligen_workflow_application_public`；round `28` 进一步删除了 [modules/workflow/CMakeLists.txt](/D:/Projects/SiligenSuite/modules/workflow/CMakeLists.txt) 中的 `siligen_workflow_application_public`，并把 `siligen_module_workflow` 重冻为显式 canonical bundle；round `29` 再把 `PlanningPortAdapters.h` 收敛为 declaration-only public header，workflow public surface 不再隐式透传 `ProcessPathFacade.h`、`MotionPlanningFacade.h` 与 `DxfPbPreparationService.h`。
...
- **`suggested_disposition`**：retarget / continue-tighten
- **`acceptance_check`**：工作流头文件目标只暴露自身的 public include，其他模块依赖由消费者按需管理；外部 app / test consumer 不再依赖 `siligen_workflow_application_public` 获取 foreign owner surface，且 `modules/workflow/CMakeLists.txt` 不再定义该 aggregate。
...
3. **machine residue 的主战场已从 bridge-domain 迁移到 public surface**：`domain/domain/machine/**` 已不再是当前事实；真正仍活跃的是 `domain/include/domain/machine/**` 里的 mixed owner surface 与其 live consumer。
4. **Application/Test 侧真值已继续前移**：`siligen_application_motion` 当前已落到 `modules/runtime-execution/application`，`siligen_control_application` / `siligen_application` 已无 live 命中；round `25` 已收掉 `siligen_workflow_application_headers` foreign-surface distributor，round `26` 完成外部 live app consumer retarget，round `27` 又完成 workflow canonical/integration tests 脱离 aggregate，round `28` 进一步删除了 `siligen_workflow_application_public` 并把 `siligen_module_workflow` 收敛为 canonical bundle，round `29` 又把 `PlanningPortAdapters.h` 从 inline payload 头收敛为 declaration-only surface，当前 application 活跃风险已缩到 `siligen_application_dispensing` keep-set 审计。

# 7. Clean-exit blockers
1. machine/calibration 的 split 轴仍未冻结：`DispenserModel` / `CalibrationProcess` 已 forward 到 `modules/coordinate-alignment/domain/machine/**`，但 `IHardwareConnectionPort` / `IHardwareTestPort` / `MachineMode` / `MachineState` 仍由 workflow public root 暴露并被 live consumer 使用。
2. application 侧当前真正活跃的阻塞已从 module-root public-surface / naming drift 进一步收敛到 `siligen_application_dispensing` keep-set 审计：`siligen_workflow_application_headers` 的 foreign-surface leak 已在 round `25` 收口，live app consumer retarget 与 workflow test-consumer cleanup 已在 round `26/27` 收口，round `28` 又已删除 `siligen_workflow_application_public` 并把 `siligen_module_workflow` 收敛为显式 canonical bundle，round `29` 又已清掉 `PlanningPortAdapters.h` 的外部 application header re-export。
3. `siligen_application` 宏 target 已退出 live code；后续 application 批次不应再把 alias 删除当主战场，而应转向 `WF-B03` application public-surface leak 与剩余 workflow live surface 的边界冻结。
...
2. `siligen_application_dispensing` 当前保留在 `PUBLIC` 的 contracts/domain keep-set 中，哪些仍是编排层必需依赖，哪些只是历史 carry-over（需结合依赖树与 targeted build 继续收敛）？
