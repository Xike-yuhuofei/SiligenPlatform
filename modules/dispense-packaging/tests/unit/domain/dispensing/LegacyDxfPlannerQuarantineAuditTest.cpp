...
// Quarantine audit for legacy DXF planner compat behavior. These assertions do not define canonical motion contracts.
TEST(LegacyDxfPlannerQuarantineAuditTest, KeepsArcSamplesOnTrueCircle) {
...
TEST(LegacyDxfPlannerQuarantineAuditTest, IsCurrentlyInsensitiveToArcToleranceRequest) {
    const auto arc = BuildQuarterArc();
    auto path_source = std::make_shared<SinglePrimitivePathSourceStub>(
        Primitive::MakeArc(arc.center, arc.radius, arc.start_angle_deg, arc.end_angle_deg, arc.clockwise));
    DispensingPlanner planner(path_source);
