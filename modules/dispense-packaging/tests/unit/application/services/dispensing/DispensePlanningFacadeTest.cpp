...
TEST(DispensePlanningFacadeTest, AssemblePlanningArtifactsUsesStableClosedLoopPhaseWithoutDuplicateTail) {
    ...
    EXPECT_TRUE(square_payload.authority_trigger_layout.spans.front().closed);
    EXPECT_EQ(
        square_payload.glue_points.size(),
        square_payload.authority_trigger_layout.spans.front().interval_count);
    EXPECT_GT(square_payload.authority_trigger_layout.spans.front().phase_mm, 0.0f);
    EXPECT_LT(
        square_payload.authority_trigger_layout.spans.front().phase_mm,
        square_payload.authority_trigger_layout.spans.front().actual_spacing_mm);
    EXPECT_GT(
        square_payload.glue_points.front().DistanceTo(square_payload.glue_points.back()),
        1e-3f);
    EXPECT_NEAR(
        square_payload.authority_trigger_layout.spans.front().phase_mm,
        rotated_payload.authority_trigger_layout.spans.front().phase_mm,
        1e-4f);
}
