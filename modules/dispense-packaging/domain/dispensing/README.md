...
- `siligen_dispense_packaging_domain_dispensing_planning_residual_headers` 与
  - 仅为 planning residual/internal target 提供 fat include 与 runtime bridge，不得重新挂回 owner/core target。
...
- `当前实现事实`
  - 本目录当前只剩 planning residual concrete，execution concrete 已迁出并删除。
- `本轮处理口径`
