...
- foreign storage 适配只允许存在于 runtime-service composition root 或模块 integration tests
- 本批次不在 application 层重新冻结 storage owner / PB owner；M1 只保留 source drawing 接入 seam，不吞回 planning preparation owner 语义
