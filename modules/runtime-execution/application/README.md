...
- runtime config consumer 统一从 `runtime_execution/contracts/configuration/IConfigurationPort.h` 引入；`ReadyZeroSpeedResolver` 固定从 `process_planning/contracts/configuration/ReadyZeroSpeedResolver.h` 引入，禁止回退到 `domain/configuration/**` legacy include。
- `process_planning/contracts/configuration/{ConfigTypes,IConfigurationPort,ValveConfig,ReadyZeroSpeedResolver}.h` 已成为 configuration 四件套的 physical owner；runtime-execution 不得再把 `process-planning/domain/configuration/**` 当作 public contract root。
