...
Result<DispensingExecutionResult> DispensingExecutionUseCase::Impl::ExecuteInternal(
    ...
    if (request.velocity_trace_enabled) {
        ...
        const std::string trace_anchor_path =
            request.source_path.empty() ? execution_package.source_path : request.source_path;
        auto resolved_trace_path = VelocityTracePathPolicy::ResolveOutputPath(
            trace_anchor_path,
            trace_settings.output_path);
        if (resolved_trace_path.IsError()) {
            return Result<DispensingExecutionResult>::Failure(resolved_trace_path.GetError());
        }
        trace_settings.output_path = resolved_trace_path.Value();
    ...
    auto execution_observer = CreateExecutionObserver(
        motion_state_port_,
        trace_settings.output_path,
        trace_settings.interval_ms,
