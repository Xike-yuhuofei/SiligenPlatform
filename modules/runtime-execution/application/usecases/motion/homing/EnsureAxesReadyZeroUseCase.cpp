...
using Siligen::RuntimeExecution::Application::Services::Motion::ReadyZeroDecision;
using Siligen::RuntimeExecution::Application::Services::Motion::ReadyZeroDecisionService;
using Siligen::RuntimeExecution::Application::Services::Motion::ReadyZeroPlannedAction;
using Siligen::RuntimeExecution::Application::Services::Motion::ReadyZeroSupervisorState;
    ...
    if (!axes_to_home.empty()) {
        ...
        for (const auto axis : axes_to_home) {
            ...
            if (!homing_success) {
                ...
                axis_result->message =
                    axis_it != home_response.axis_results.end() ? axis_it->message : "Homing failed";
            }
        }
    ...
    if (!axes_to_go_home.empty()) {
        ...
        for (const auto& snapshot : final_snapshots) {
            ...
            axis_result->reason_code =
                snapshot.decision.reason_code.empty() ? "zero_position_not_reached" : snapshot.decision.reason_code;
            axis_result->message = snapshot.decision.message.empty() ? "Axis did not reach zero" : snapshot.decision.message;
