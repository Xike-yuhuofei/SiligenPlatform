...
void AttachHomingState(const std::shared_ptr<Domain::Motion::Ports::IHomingPort>& homing_port,
...
Result<Domain::Motion::Ports::CoordinateSystemStatus> MotionMonitoringUseCase::GetCoordinateSystemStatus(
...
Result<Siligen::RuntimeExecution::Contracts::Motion::IOStatus> MotionMonitoringUseCase::ReadDigitalInputStatus(
    int16 channel) const {
    auto validation = ValidateChannelNumber(channel);
    if (validation.IsError()) {
        return Result<Siligen::RuntimeExecution::Contracts::Motion::IOStatus>::Failure(validation.GetError());
    }
...
Result<Siligen::RuntimeExecution::Contracts::Motion::IOStatus> MotionMonitoringUseCase::ReadDigitalOutputStatus(
    int16 channel) const {
    auto validation = ValidateChannelNumber(channel);
    if (validation.IsError()) {
        return Result<Siligen::RuntimeExecution::Contracts::Motion::IOStatus>::Failure(validation.GetError());
...
Result<void> MotionMonitoringUseCase::StartStatusUpdate(int32 interval_ms) {
    ...
    std::lock_guard<std::mutex> lock(status_update_lifecycle_mutex_);
    status_update_thread_ = std::thread(&MotionMonitoringUseCase::StatusUpdateLoop, this);
    return Result<void>::Success();
}
...
void MotionMonitoringUseCase::StopStatusUpdate() {
    ...
    bool called_from_worker_thread = false;
    {
        ...
        if (status_update_thread_.joinable()) {
            if (status_update_thread_.get_id() == std::this_thread::get_id()) {
                // When the worker thread requests stop from inside a callback, let the outer thread own the join.
...
void MotionMonitoringUseCase::StatusUpdateTimer() {
    if (motion_state_port_) {
        auto all_status_result = GetAllAxesMotionStatus();
        if (all_status_result.IsSuccess()) {
            motion_status_failure_count_.store(0);
            motion_status_failure_logged_.store(false);
            for (size_t i = 0; i < all_status_result.Value().size(); ++i) {
                auto axis_id = FromIndex(static_cast<int16>(i));
                if (IsValid(axis_id)) {
                    NotifyMotionStatusUpdate(axis_id, all_status_result.Value()[i]);
                }
            }
        } else {
            auto failure_count = motion_status_failure_count_.fetch_add(1) + 1;
            if (!motion_status_failure_logged_.exchange(true)) {
                SILIGEN_LOG_ERROR_FMT_HELPER(
                    "[MotionMonitoringUseCase] failed to refresh motion status (count=%u): %s",
                    failure_count,
                    all_status_result.GetError().GetMessage().c_str());
            }
        }
    }

    if (io_port_) {
        auto all_io_result = ReadAllDigitalInputStatus();
        if (all_io_result.IsSuccess()) {
            io_status_failure_count_.store(0);
            io_status_failure_logged_.store(false);
            for (const auto& signal : all_io_result.Value()) {
                NotifyIOStatusUpdate(signal);
            }
        } else {
            auto failure_count = io_status_failure_count_.fetch_add(1) + 1;
            if (!io_status_failure_logged_.exchange(true)) {
                SILIGEN_LOG_ERROR_FMT_HELPER(
                    "[MotionMonitoringUseCase] failed to refresh IO status (count=%u): %s",
                    failure_count,
                    all_io_result.GetError().GetMessage().c_str());
            }
        }
    }
}
