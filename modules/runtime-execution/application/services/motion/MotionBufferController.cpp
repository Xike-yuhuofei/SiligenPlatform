...
MotionBufferController::MotionBufferController(std::shared_ptr<IInterpolationPort> interpolation_port)
...
int32 MotionBufferController::GetBufferSpace(short crd_num) {
    ...
    const int32 space = static_cast<int32>(result.Value());
...
bool MotionBufferController::CheckBufferHealth(short crd_num, uint32& underflow_count) {
    const int32 space = GetBufferSpace(crd_num);
    if (space < 10) {
        ++underflow_count;
