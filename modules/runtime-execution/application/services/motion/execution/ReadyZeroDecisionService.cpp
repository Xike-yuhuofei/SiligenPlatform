#include "ReadyZeroDecisionService.h"
...
using MotionState = Ports::MotionState;

bool IsFaulted(const Ports::MotionStatus& status) {
...
bool IsMoving(const Ports::MotionStatus& status) {
    return status.state == MotionState::MOVING;
}

bool IsHoming(const Ports::MotionStatus& status) {
...
ReadyZeroDecision ReadyZeroDecisionService::EvaluateAxis(LogicalAxisId axis,
                                                         const Ports::MotionStatus& status,
    ...
    if (!decision.homed) {
        ...
        decision.message = status.homing_state == "failed"
            ? "Axis homing failed previously; rehome required"
            : "Axis is not homed";
        return decision;
    }
...
float32 ReadyZeroDecisionService::ExtractAxisPositionMm(LogicalAxisId axis, const Ports::MotionStatus& status) {
