#include "runtime_execution/application/services/calibration/CalibrationExecutionService.h"
...
namespace {
constexpr const char* kErrorSource = "CalibrationExecutionService";
}  // namespace

CalibrationExecutionService::CalibrationExecutionService(
...
void CalibrationExecutionService::InitializeValidTransitions() noexcept {
    valid_transitions_ = {
...
bool CalibrationExecutionService::IsValidTransition(
    CalibrationExecutionState from,
    CalibrationExecutionState to) const noexcept {
    return valid_transitions_.count({from, to}) > 0;
}

Result<void> CalibrationExecutionService::Start(const CalibrationExecutionRequest& request) noexcept {
...
Result<void> CalibrationExecutionService::ProcessNextStep() noexcept {
...
Result<void> CalibrationExecutionService::Cancel() noexcept {
    if (current_state_ == CalibrationExecutionState::Idle) {
        return Result<void>::Failure(
            Error(ErrorCode::INVALID_STATE, "Calibration not started", kErrorSource));
    }
...
void CalibrationExecutionService::Reset() noexcept {
    current_state_ = CalibrationExecutionState::Idle;
    context_ = CalibrationContext{};
}

Result<void> CalibrationExecutionService::TransitionTo(CalibrationExecutionState new_state) noexcept {
...
Result<void> CalibrationExecutionService::FinalizeResult(
...
Result<void> CalibrationExecutionService::FailWithError(const Error& error) noexcept {
...
Result<void> CalibrationExecutionService::ProcessValidation() noexcept {
    if (!execution_port_) {
        return Result<void>::Failure(
...
Result<void> CalibrationExecutionService::ProcessExecution() noexcept {
    if (!execution_port_) {
        return Result<void>::Failure(
            Error(ErrorCode::PORT_NOT_INITIALIZED, "Calibration execution port not initialized", kErrorSource));
    }
...
Result<void> CalibrationExecutionService::ProcessVerification() noexcept {
    if (!execution_port_) {
        return Result<void>::Failure(
            Error(ErrorCode::PORT_NOT_INITIALIZED, "Calibration execution port not initialized", kErrorSource));
    }
...
bool CalibrationExecutionService::IsTerminalState(CalibrationExecutionState state) noexcept {
...
const char* CalibrationExecutionService::StateToString(CalibrationExecutionState state) noexcept {
    switch (state) {
        case CalibrationExecutionState::Idle:
            return "IDLE";
        case CalibrationExecutionState::Validating:
