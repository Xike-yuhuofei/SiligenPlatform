...
LegacyDispenserModel::LegacyDispenserModel()
...
Result<void> LegacyDispenserModel::SetState(DispenserState state) {
...
DispenserState LegacyDispenserModel::GetState() const {
    return current_state_;
}

Result<bool> LegacyDispenserModel::CanDispense() const {
    const bool can_dispense =
        current_state_ == DispenserState::READY || current_state_ == DispenserState::PAUSED;
    return Result<bool>::Success(can_dispense);
}

bool LegacyDispenserModel::HasError() const {
    return current_state_ == DispenserState::ERROR_STATE;
}

Result<void> LegacyDispenserModel::ClearError() {
...
Result<void> LegacyDispenserModel::AddTask(const DispensingTask& task) {
    if (!task.Validate()) {
        return Result<void>::Failure(
    ...
    if (static_cast<int32_t>(pending_tasks_.size()) >= MAX_PENDING_TASKS) {
        return Result<void>::Failure(Error(ErrorCode::INVALID_STATE, "Task queue is full", "DispenserModel"));
    }
...
Result<void> LegacyDispenserModel::StartTask(const std::string& task_id) {
    auto it = std::find_if(
...
Result<void> LegacyDispenserModel::PauseCurrentTask() {
...
Result<void> LegacyDispenserModel::ResumeCurrentTask() {
...
Result<void> LegacyDispenserModel::CancelTask(const std::string& task_id) {
...
Result<void> LegacyDispenserModel::CompleteCurrentTask() {
...
Result<DispensingTask> LegacyDispenserModel::GetCurrentTask() const {
    if (current_task_.task_id.empty()) {
...
Result<std::vector<DispensingTask>> LegacyDispenserModel::GetPendingTasks() const {
    return Result<std::vector<DispensingTask>>::Success(pending_tasks_);
}

Result<int32_t> LegacyDispenserModel::GetTaskQueueSize() const {
    return Result<int32_t>::Success(static_cast<int32_t>(pending_tasks_.size()));
}

Result<void> LegacyDispenserModel::ClearAllTasks() {
    pending_tasks_.clear();
    current_task_ = DispensingTask();
    return Result<void>::Success();
}

Result<void> LegacyDispenserModel::SetMotionConfig(const Siligen::Shared::Types::MotionConfig& config) {
    motion_config_ = config;
    return Result<void>::Success();
}

const Siligen::Shared::Types::MotionConfig& LegacyDispenserModel::GetMotionConfig() const {
    return motion_config_;
}

Result<void> LegacyDispenserModel::SetConnectionConfig(
    const Siligen::Shared::Types::ConnectionConfig& config) {
    connection_config_ = config;
    return Result<void>::Success();
}

const Siligen::Shared::Types::ConnectionConfig& LegacyDispenserModel::GetConnectionConfig() const {
    return connection_config_;
}

int32_t LegacyDispenserModel::GetCompletedTaskCount() const {
    return total_completed_tasks_;
}

double LegacyDispenserModel::GetTotalRunningTime() const {
    return total_running_time_;
}

double LegacyDispenserModel::GetTotalDispensingLength() const {
    return total_dispensing_length_;
}

Result<void> LegacyDispenserModel::ResetStatistics() {
    total_completed_tasks_ = 0;
    total_running_time_ = 0.0;
    total_dispensing_length_ = 0.0;
    return Result<void>::Success();
...
Result<void> LegacyDispenserModel::ValidateStateTransition(DispenserState from_state, DispenserState to_state) {
    switch (from_state) {
        case DispenserState::UNINITIALIZED:
            if (to_state == DispenserState::INITIALIZING || to_state == DispenserState::EMERGENCY_STOP) {
                return Result<void>::Success();
            }
            break;
        case DispenserState::INITIALIZING:
            if (to_state == DispenserState::READY || to_state == DispenserState::ERROR_STATE ||
                to_state == DispenserState::EMERGENCY_STOP) {
                return Result<void>::Success();
            }
            break;
        case DispenserState::READY:
            if (to_state == DispenserState::DISPENSING || to_state == DispenserState::ERROR_STATE ||
                to_state == DispenserState::EMERGENCY_STOP) {
                return Result<void>::Success();
            }
            break;
        case DispenserState::DISPENSING:
            if (to_state == DispenserState::PAUSED || to_state == DispenserState::READY ||
                to_state == DispenserState::ERROR_STATE || to_state == DispenserState::EMERGENCY_STOP) {
                return Result<void>::Success();
            }
            break;
        case DispenserState::PAUSED:
            if (to_state == DispenserState::DISPENSING || to_state == DispenserState::READY ||
                to_state == DispenserState::ERROR_STATE || to_state == DispenserState::EMERGENCY_STOP) {
                return Result<void>::Success();
            }
            break;
        case DispenserState::ERROR_STATE:
            if (to_state == DispenserState::READY || to_state == DispenserState::EMERGENCY_STOP) {
                return Result<void>::Success();
            }
            break;
    ...
    return Result<void>::Failure(Error(ErrorCode::INVALID_STATE, "Invalid state transition", "DispenserModel"));
}

Result<void> LegacyDispenserModel::ValidateTaskStart(const DispensingTask& task) const {
...
std::string LegacyDispenserModel::ToJson() const {
...
std::string LegacyDispenserModel::ToString() const {
    std::ostringstream oss;
...
bool LegacyDispenserModel::IsStateTransitionValid(DispenserState from_state, DispenserState to_state) const {
    return ValidateStateTransition(from_state, to_state).IsSuccess();
}

void LegacyDispenserModel::UpdateStatistics(const DispensingTask& completed_task) {
    ...
    if (completed_task.started_at.time_since_epoch().count() > 0 &&
        ...
        const auto duration = completed_task.completed_at - completed_task.started_at;
...
double LegacyDispenserModel::CalculatePathLength(const std::vector<Point2D>& path) const {
    if (path.size() < 2) {
        return 0.0;
    }
