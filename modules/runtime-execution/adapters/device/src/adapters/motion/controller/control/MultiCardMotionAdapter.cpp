...
namespace {
// HOME 只表达回零参考边界，不再对外伪装成通用负向硬限位。
constexpr bool kUseHomeSignalAsBoundary = true;
}
...
Result<void> MultiCardMotionAdapter::PrepareAxisForJog(short axis,
    ...
    if (kUseHomeSignalAsBoundary) {
        auto boundary_result = ValidateHomeBoundaryForDirection(axis, direction, operation);
        if (boundary_result.IsError()) {
...
Result<void> MultiCardMotionAdapter::MoveAxisToPosition(LogicalAxisId axis_id, float32 position, float32 velocity) {
    ...
    if (kUseHomeSignalAsBoundary) {
        const int16 direction = (position > current_pos) ? 1 : ((position < current_pos) ? -1 : 0);
        auto boundary_result = ValidateHomeBoundaryForDirection(axis, direction, "MoveAxisToPosition");
        if (boundary_result.IsError()) {
            return boundary_result;
        }
    }
...
Result<void> MultiCardMotionAdapter::RelativeMove(LogicalAxisId axis_id, float32 distance, float32 velocity) {
    ...
    if (kUseHomeSignalAsBoundary) {
        const int16 direction = (distance > 0.0f) ? 1 : ((distance < 0.0f) ? -1 : 0);
        auto boundary_result = ValidateHomeBoundaryForDirection(axis, direction, "RelativeMove");
        if (boundary_result.IsError()) {
            return boundary_result;
        }
    }
