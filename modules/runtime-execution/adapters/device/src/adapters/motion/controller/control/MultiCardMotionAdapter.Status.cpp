...
namespace {
// HOME 只保留为 home_signal，不再映射为 hard_limit_negative。
...
Result<MotionStatus> MultiCardMotionAdapter::GetAxisStatus(LogicalAxisId axis_id) const {
    ...
    if (ret == 0) {
        ...
        const bool home_status_bit = (hw_status & AXIS_STATUS_HOME_SWITCH) != 0;
        ...
        status.home_failed = home_failed;
        status.home_signal = home_status_bit;
        status.index_signal = (hw_status & AXIS_STATUS_INDEX) != 0;

        auto raw_home_result = IsHomeSensorTriggeredRaw(axis);
        if (raw_home_result.IsSuccess()) {
            if (home_status_bit != raw_home_result.Value()) {
                SILIGEN_LOG_WARNING("GetAxisStatus: HOME raw/state mismatch for axis " + std::to_string(axis) +
                                    " raw=" + std::to_string(raw_home_result.Value()) +
                                    " status=" + std::to_string(home_status_bit));
            }
            status.home_signal = home_status_bit || raw_home_result.Value();
        } else {
            SILIGEN_LOG_WARNING("GetAxisStatus: failed to read HOME raw input for axis " + std::to_string(axis) +
                                ": " + raw_home_result.GetError().GetMessage());
        }
...
Result<bool> MultiCardMotionAdapter::ReadLimitStatus(LogicalAxisId axis_id, bool positive) {
    auto axis_result = ResolveAxis(axis_id, "ReadLimitStatus");
    if (axis_result.IsError()) {
        return Result<bool>::Failure(axis_result.GetError());
    }

    const short axis = axis_result.Value();

    if (!kExposeHomeAsHardLimit && !positive) {
