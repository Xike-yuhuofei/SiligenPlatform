...
namespace {
// HOME 只保留为独立参考边界信号，不再对外映射成通用负向硬限位。
}
...
Result<bool> MultiCardMotionAdapter::IsHomeSensorTriggeredRaw(short axis) const {
    if (!ValidateAxis(axis)) {
        return Result<bool>::Failure(
            Shared::Types::Error(Shared::Types::ErrorCode::INVALID_AXIS,
                                 FormatErrorMessage("IsHomeSensorTriggeredRaw", axis, -1)));
    }
    ...
    return Result<bool>::Success(state_result.Value()[axis]);
}

Result<void> MultiCardMotionAdapter::ValidateHomeBoundaryForDirection(short axis,
                                                                      int16 direction,
    ...
    auto home_triggered_result = IsHomeSensorTriggeredRaw(axis);
    if (home_triggered_result.IsError()) {
        return Result<void>::Failure(home_triggered_result.GetError());
    }
    if (!home_triggered_result.Value()) {
        return Result<void>::Success();
    }

    const short sdk_axis = ToSdkAxis(axis);
    long hw_status = 0;
    if (hardware_wrapper_->MC_GetSts(sdk_axis, &hw_status) == 0) {
        const bool axis_homed = (hw_status & AXIS_STATUS_HOME_SUCESS) != 0;
        if (axis_homed) {
            return Result<void>::Success();
        }
    }
    ...
    return Result<void>::Failure(
        Shared::Types::Error(
            ...
            std::string(operation) +
                ": HOME boundary is active, only positive escape motion is allowed"));
}
