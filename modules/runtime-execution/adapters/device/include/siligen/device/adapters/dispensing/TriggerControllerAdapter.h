 ...
 * @brief 触发控制端口适配器
 *
 * 基于 device-adapters 内部 trigger concrete 实现 ITriggerControllerPort。
 */
class TriggerControllerAdapter final : public ITriggerControllerPort {
   public:
    explicit TriggerControllerAdapter(
        std::shared_ptr<Siligen::Infrastructure::Adapters::Hardware::HardwareTestAdapter> hardware_test_port);
   ...
   private:
    ...
    std::shared_ptr<Siligen::Infrastructure::Adapters::Hardware::HardwareTestAdapter> hardware_test_port_;
