...
constexpr const char* kErrorSource = "DispenserModelMachineExecutionStateBackend";
...
Result<std::shared_ptr<DispenserModel>> EnsureModel(const std::shared_ptr<DispenserModel>& dispenser_model) {
    if (!dispenser_model) {
...
DispenserModelMachineExecutionStateBackend::DispenserModelMachineExecutionStateBackend()
...
Result<MachineExecutionSnapshot> DispenserModelMachineExecutionStateBackend::ReadSnapshot() const {
...
Result<void> DispenserModelMachineExecutionStateBackend::ClearPendingTasks() {
    auto model_result = EnsureModel(dispenser_model_);
    if (model_result.IsError()) {
...
Result<void> DispenserModelMachineExecutionStateBackend::TransitionToEmergencyStop() {
    auto model_result = EnsureModel(dispenser_model_);
    if (model_result.IsError()) {
        return Result<void>::Failure(model_result.GetError());
    }
...
Result<void> DispenserModelMachineExecutionStateBackend::RecoverToUninitialized() {
    auto model_result = EnsureModel(dispenser_model_);
    if (model_result.IsError()) {
        return Result<void>::Failure(model_result.GetError());
    }
