...
constexpr const char* kErrorSource = "WorkflowMachineExecutionStateBackend";
...
WorkflowMachineExecutionStateBackend::WorkflowMachineExecutionStateBackend()
...
Result<MachineExecutionSnapshot> WorkflowMachineExecutionStateBackend::ReadSnapshot() const {
...
Result<void> WorkflowMachineExecutionStateBackend::ClearPendingTasks() {
    auto model_result = EnsureModel(dispenser_model_);
    if (model_result.IsError()) {
...
Result<void> WorkflowMachineExecutionStateBackend::TransitionToEmergencyStop() {
    auto model_result = EnsureModel(dispenser_model_);
    if (model_result.IsError()) {
        return Result<void>::Failure(model_result.GetError());
    }
...
Result<void> WorkflowMachineExecutionStateBackend::RecoverToUninitialized() {
    auto model_result = EnsureModel(dispenser_model_);
    if (model_result.IsError()) {
        return Result<void>::Failure(model_result.GetError());
    }
