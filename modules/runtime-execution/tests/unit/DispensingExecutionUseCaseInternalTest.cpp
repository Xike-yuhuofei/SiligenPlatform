...
TEST(DispensingExecutionUseCaseInternalTest, RunningTaskUsesEstimatedProgressFallback) {
    ...
    use_case->session_store_.tasks_[context->task_id] = context;
    use_case->session_store_.active_task_id_ = context->task_id;
...
TEST(DispensingExecutionUseCaseInternalTest, RejectsOperationsOnNonActiveTask) {
    ...
    use_case->session_store_.tasks_[active_context->task_id] = active_context;
    use_case->session_store_.active_task_id_ = active_context->task_id;
    ...
    use_case->session_store_.tasks_[stale_context->task_id] = stale_context;
...
TEST(DispensingExecutionUseCaseInternalTest, CleanupExpiredTasksKeepsActiveRunningTask) {
    ...
    use_case->session_store_.tasks_[running_context->task_id] = running_context;
    use_case->session_store_.active_task_id_ = running_context->task_id;
    ...
    use_case->session_store_.tasks_[completed_context->task_id] = completed_context;

    use_case->CleanupExpiredTasks();

    EXPECT_NE(
        use_case->session_store_.tasks_.find(running_context->task_id),
        use_case->session_store_.tasks_.end());
    EXPECT_EQ(use_case->session_store_.active_task_id_, running_context->task_id);
    EXPECT_EQ(
        use_case->session_store_.tasks_.find(completed_context->task_id),
        use_case->session_store_.tasks_.end());
}
