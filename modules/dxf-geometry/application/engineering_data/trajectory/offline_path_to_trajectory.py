...
def find_segment(cum_lengths: List[float], s: float) -> int:
    # linear scan is fine for current sizes; can optimize with bisect later
...
def build_report(samples: List[TrajectorySample],
    ...
    last_t = None
    for s in samples:
        v = abs(s.scalar_velocity)
        a = abs(s.scalar_acceleration)
        max_v = max(max_v, v)
        max_a = max(max_a, a)
        if last_acc is not None and last_t is not None:
            dt = max(1e-9, s.t - last_t)
            j = abs(s.scalar_acceleration - last_acc) / dt
            max_j = max(max_j, j)
            if jerk_enforced and j > jmax + 1e-6:
                violations += 1
        if v > vmax + 1e-6 or a > amax + 1e-6:
            violations += 1
        last_acc = s.scalar_acceleration
        last_t = s.t
...
def build_trajectory_artifact(points: List[Point2D],
    ...
    if res not in (Result.Working, Result.Finished):
        raise RuntimeError(f"Ruckig calculate failed: {res}")
