...
- 兼容别名与已知缺口不在本文件重复维护，统一记录于：
  - `D:\Projects\SiligenSuite\packages\application-contracts\mappings\protocol-mapping.md`
  - `D:\Projects\SiligenSuite\packages\application-contracts\mappings\compatibility-overrides.json`

## 4. app 薄入口规则

- `control-tcp-server` main 只允许：
  - 解析命令行
  - 调用 `BuildContainer(...)`
  - 通过 `BuildTcpFacadeBundle(*container)` 获取 facade bundle
  - 通过 `TcpServerHost` 启动 TCP server
- main 不允许再直接 new `TcpCommandDispatcher` 或 include `modules/control-gateway/src/*`
