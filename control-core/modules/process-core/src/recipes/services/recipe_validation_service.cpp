...
Siligen::SharedKernel::VoidResult MakeValidationError(const std::string& message) {
    return Siligen::SharedKernel::VoidResult::Failure(
        Siligen::SharedKernel::Error(Siligen::SharedKernel::ErrorCode::RECIPE_VALIDATION_FAILED,
                                     message,
                                     "RecipeValidation"));
}

const ValueObjects::ParameterSchemaEntry* FindSchemaEntry(
    const std::vector<ValueObjects::ParameterSchemaEntry>& entries,
...
Siligen::SharedKernel::VoidResult RecipeValidationService::ValidateRecipe(const Aggregates::Recipe& recipe) const {
...
Siligen::SharedKernel::VoidResult RecipeValidationService::ValidateRecipeVersion(
...
Siligen::SharedKernel::VoidResult RecipeValidationService::ValidateParameters(
    const Aggregates::RecipeVersion& version,
    const ValueObjects::ParameterSchema& schema) const {
    ...
    for (const auto& pair : values) {
        ...
        switch (schema_entry->value_type) {
            case ValueObjects::ParameterValueType::Integer: {
                ...
                if (schema_entry->constraints.min_value.has_value() && value < schema_entry->constraints.min_value.value()) {
                    return MakeValidationError("Parameter below min: " + param.key);
                }
                if (schema_entry->constraints.max_value.has_value() && value > schema_entry->constraints.max_value.value()) {
                    return MakeValidationError("Parameter above max: " + param.key);
                }
                break;
            }
            case ValueObjects::ParameterValueType::Float: {
