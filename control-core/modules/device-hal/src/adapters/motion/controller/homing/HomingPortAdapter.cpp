...
#include "modules/device-hal/src/adapters/motion/controller/homing/HomingSupport.h"
...
Result<void> HomingPortAdapter::startHoming(const std::vector<LogicalAxisId>& axes) {
    ...
    auto request_check =
        HomingSupport::ValidateStartHomingRequest(axes, [this]() { return isConnected(); }, "HomingPortAdapter");
...
Result<void> HomingPortAdapter::EscapeFromHomeLimit(int axis, short axisNum, const HomingConfig& config) {
