...
// 条件编译：仅在存在 MultiCard.lib 时包含厂商SDK头文件
...
/**
 * @brief 真实 MultiCard 硬件包装器
 *
 * 包装真实的 MultiCard SDK，提供 IMultiCardWrapper 接口实现。
 *
 * 设计原则:
 * - 直接委托给 MultiCard C++ 对象
 * - 零拷贝、零抽象开销（虚函数调用除外）
 * - 错误码直接透传（不转换，由上层适配器处理）
 */
class RealMultiCardWrapper : public IMultiCardWrapper {
   public:
    /**
     * @brief 构造函数
     * @param multicard 已存在的 MultiCard 对象（可选）
     *
     * 如果未提供 multicard，则构造函数会创建新实例。
     * 这样支持依赖注入和单元测试。
     */
    explicit RealMultiCardWrapper(std::shared_ptr<MultiCard> multicard = nullptr);

    /**
     * @brief 析构函数
     *
     * 注意：不自动调用 MC_Close()，由上层适配器管理生命周期。
     */
    ~RealMultiCardWrapper() override = default;

    // 禁止拷贝和移动
    RealMultiCardWrapper(const RealMultiCardWrapper&) = delete;
    RealMultiCardWrapper& operator=(const RealMultiCardWrapper&) = delete;
    RealMultiCardWrapper(RealMultiCardWrapper&&) = delete;
    RealMultiCardWrapper& operator=(RealMultiCardWrapper&&) = delete;
    ...
    // 底层访问
    void* GetUnderlyingMultiCard() noexcept override;
    

   private:
    std::shared_ptr<MultiCard> multicard_;  ///< 真实 MultiCard SDK 对象
    std::map<int, std::vector<TCrdData>> lookahead_buffers_;
};
