...
### 1.2 通信架构
- **通信方式**: 以太网 UDP/IP 通信（必须使用UDP，禁止使用TCP）
- **支持卡数**: 最多支持16张控制卡同时连接
- **轴数支持**: 每张卡最多16轴，总计最多32轴
- **坐标系数**: 最多16个坐标系
- **协议要求**: 仅支持UDP协议，确保实时性和低延迟
...
#### UDP协议特性
- **低延迟**: UDP协议确保运动控制命令的实时传输
- **无连接开销**: 避免TCP的握手和连接维护开销
- **广播支持**: 支持一对多的控制命令广播
- **实时性保证**: 确保运动控制的毫秒级响应要求

#### 网络环境要求
1. **局域网环境**: 控制卡和PC必须在同一局域网内
2. **网络质量**: 确保网络延迟<1ms，丢包率<0.1%
3. **交换机配置**: 使用工业级网络交换机，启用QoS优先级
4. **禁用TCP**: 系统不支持TCP协议，禁止尝试TCP连接

#### UDP端口配置
```cpp
// 正确的UDP端口配置
short pcPort = 0;           // 0表示系统自动分配UDP端口
short cardPort = 0;         // 0表示控制卡自动分配UDP端口

// MultiCard.dll会自动创建UDP socket
// 无需手动管理端口号，避免冲突
```
...
| nPCEthernetPort | unsigned short | PC端端口号 | 0 |
| cCardEthernetIP | char* | 控制卡IP地址 | "192.168.0.1" |
| nCardEthernetPort | unsigned short | 控制卡端口号 | 0 |
...
### 4.3 端口管理原则
1. **自动分配**: 使用端口0让系统自动分配端口号
2. **一致性**: PC端和控制卡端都使用端口0
3. **唯一性**: 系统会确保每张控制卡获得唯一端口
...
int ConnectMultipleCards()
{
    ...
    // 连接第一张卡
    result = g_MultiCard.MC_Open(1, "192.168.0.200", 0, "192.168.0.1", 0);
    if (result != 0) return result;

    // 连接第二张卡
    result = g_MultiCard2.MC_Open(2, "192.168.0.200", 0, "192.168.0.2", 0);
    if (result != 0) return result;

    // 连接第三张卡
    result = g_MultiCard3.MC_Open(3, "192.168.0.200", 0, "192.168.0.3", 0);
    if (result != 0) return result;
...
# 使用Wireshark抓包分析
# 过滤条件: ip.addr == 192.168.0.1 && udp

# 测试UDP连通性
powershell -Command "Test-NetConnection -ComputerName 192.168.0.1 -Port 0"
```
...
2. **网络配置**: 静态IP和UDP端口自动分配策略
3. **连接实现**: MC_Open函数的UDP连接方法
4. **多卡管理**: 全局对象声明和管理机制
5. **状态监控**: 实时状态更新和错误检测
6. **代码示例**: 完整的封装类和工具类
7. **故障排除**: 常见问题的诊断和UDP通信专项解决方案

**重要提醒**: 本系统仅支持UDP协议通信，禁止使用TCP协议。确保网络环境满足UDP实时通信要求。
