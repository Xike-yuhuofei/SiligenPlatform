...
struct DispensingRuntimeOverrides {
    ...
    Result<void> Validate() const noexcept {
        if (dispensing_speed_mm_s.has_value() && *dispensing_speed_mm_s < 0.0f) {
            return Result<void>::Failure(Error(ErrorCode::INVALID_PARAMETER, "dispensing_speed_mm_s不能为负数"));
        }
        if (dry_run_speed_mm_s.has_value() && *dry_run_speed_mm_s < 0.0f) {
            return Result<void>::Failure(Error(ErrorCode::INVALID_PARAMETER, "dry_run_speed_mm_s不能为负数"));
        }
        if (rapid_speed_mm_s.has_value() && *rapid_speed_mm_s < 0.0f) {
            return Result<void>::Failure(Error(ErrorCode::INVALID_PARAMETER, "rapid_speed_mm_s不能为负数"));
        }
        const bool has_dispensing_speed =
            (dispensing_speed_mm_s.has_value() && *dispensing_speed_mm_s > 0.0f);
        const bool has_dry_run_speed =
            (dry_run_speed_mm_s.has_value() && *dry_run_speed_mm_s > 0.0f);
        if (!has_dispensing_speed && !has_dry_run_speed) {
            return Result<void>::Failure(Error(ErrorCode::INVALID_PARAMETER, "点胶速度与空走速度至少提供一个"));
        }
        if (acceleration_mm_s2.has_value() && *acceleration_mm_s2 < 0.0f) {
        ...
        if (velocity_guard_grace_ms < 0 || velocity_guard_interval_ms < 0 || velocity_guard_max_consecutive < 0) {
            return Result<void>::Failure(
                Error(ErrorCode::INVALID_PARAMETER,
