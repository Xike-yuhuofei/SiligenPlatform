...
#define AXIS_STATUS_NEG_SOFT_LIMIT      (0x00000008)	//负软限位触发标志（规划位置小于负向软限位时置1）
...
typedef struct _LookAheadPrm
{
	...
	double dSpeedMax[32];	    //各轴的最大速度(p/ms)
	double dAccMax[32];			//各轴的最大加速度
	double dMaxStepSpeed[32];   //各轴的最大速度变化量（相当于启动速度）
	double dScale[32];			//各轴的脉冲当量
...
typedef struct _DeltaParm{ 
	long  lPlusePerCircle[3];   //关节电机每圈脉冲数，单位脉冲
	double dRotateAngle[3];     //关节平面相对XZ平面的旋转角度，单位弧度
...
typedef struct _XYZAC_PRRA{
	double dCX;					//C轴旋转中心X坐标，单位mm
	double dCY;					//C轴旋转中心Y坐标，单位mm
	double dAY;					//A轴旋转中心Y坐标，单位mm
	double dAZ;					//A轴旋转中心Z坐标，单位mm
	long lPlusePerCircle[5];    //各轴每圈脉冲数量
	double dPitch[5];           //各轴螺距,单位毫米
}XYZAC_PARM;

//XYZTATC双摆头参数
typedef struct _XYZTATC_PRRA{
	double dDX;                 //C在零度时，旋转关节中心相对Z轴的距离X,单位mm
	double dDY;					//C在零度时，旋转关节中心相对Z轴的距离Y,单位mm
	double dR;                  //末端旋转半径，单位mm
	double dOrgAngleC;          //回零完成后，C轴角度，单位度，取值范围0~360
	long lPlusePerCircle[5];    //各轴每圈脉冲数量
	double dPitch[5];           //各轴螺距,单位毫米
}XYZTATC_PARM;
...
private:
	...
	TCrdPrmEx m_LookAheadCrdPrm[CRDSYS_MAX_COUNT];//模块变量，发送坐标系参数时存放一份坐标系参数，主要给前瞻缓冲区用
