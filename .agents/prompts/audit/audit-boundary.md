...
- Verify whether module boundaries are explicit and enforceable.
- Check for information leaks across apps, shared, adapters, contracts, and neighbor modules.
- Check whether consumers depend on public surfaces instead of internal implementation details.
...
- Required fixed fields:
  ...
  - `run_id` = `{{RUN_ID}}`
  - `commit_sha` = `{{COMMIT_SHA}}`
- `findings` must include only boundary issues.
