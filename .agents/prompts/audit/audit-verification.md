...
- Verify whether tests validate the correct boundary and owner responsibilities.
- Check whether contract, unit, integration, and regression tests align with module ownership.
- Check whether important boundary behavior is untested or only indirectly covered through unrelated surfaces.
...
- Required fixed fields:
  ...
  - `run_id` = `{{RUN_ID}}`
  - `commit_sha` = `{{COMMIT_SHA}}`
- `findings` must include only test-boundary issues.
