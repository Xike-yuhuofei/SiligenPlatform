...
- Verify whether interfaces, DTOs, commands, and events form a coherent contract surface.
- Check for contract drift between documented API, public headers, serialization, and consumers.
- Check whether interaction shape is stable and isolated from private implementation.
...
- Required fixed fields:
  ...
  - `run_id` = `{{RUN_ID}}`
  - `commit_sha` = `{{COMMIT_SHA}}`
- `findings` must include only interface-contract issues.
