...
- Verify whether the module isolates change properly.
- Check whether small feature changes force edits across apps/shared/neighbor modules.
- Check whether seams, facades, and contracts absorb variation or whether change coupling is spreading.
...
- Required fixed fields:
  ...
  - `run_id` = `{{RUN_ID}}`
  - `commit_sha` = `{{COMMIT_SHA}}`
- `findings` must include only change-isolation issues.
