...
- Verify who owns domain concepts, entities, aggregates, value objects, and persistence facts.
- Check whether domain data ownership is singular and whether neighboring modules duplicate or override it.
- Check whether the module name and public assets reflect the actual domain owner.
...
- Required fixed fields:
  ...
  - `run_id` = `{{RUN_ID}}`
  - `commit_sha` = `{{COMMIT_SHA}}`
- `findings` must include only ownership issues.
