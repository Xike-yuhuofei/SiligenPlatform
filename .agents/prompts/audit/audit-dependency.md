...
- Verify dependency direction and stability.
- Check whether the module depends inward on more stable contracts, not outward on volatile implementation detail.
- Check build targets, includes, imports, and references that invert intended ownership.
...
- Required fixed fields:
  ...
  - `run_id` = `{{RUN_ID}}`
  - `commit_sha` = `{{COMMIT_SHA}}`
- `findings` must include only dependency-direction issues.
