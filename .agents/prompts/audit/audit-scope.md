...
- Verify whether the module has a clear responsibility and semantic closure.
- Check whether the module name, public API, owner objects, and documented intent align.
- Check whether live behavior is still owned by the module or leaked into apps/shared/other modules.
...
- Required fixed fields:
  ...
  - `run_id` = `{{RUN_ID}}`
  - `commit_sha` = `{{COMMIT_SHA}}`
- `findings` must include only responsibility-scope issues.
